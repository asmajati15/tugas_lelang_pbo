...
(1, 'Wizard Storm', '2023-02-10', 100, 'test', 1);
...
INSERT INTO `tb_lelang` (`id_lelang`, `id_barang`, `tanggal_barang`, `harga_akhir`, `id_user`, `id_petugas`, `status_lelang`) VALUES
(1, 1, '2023-02-08', 110, 1, 1, 1),
(2, 1, '2023-02-08', 120, 1, 1, 1);

--
-- Triggers `tb_lelang`
--
DELIMITER $$
CREATE TRIGGER `insert_lelang` AFTER INSERT ON `tb_lelang` FOR EACH ROW INSERT INTO history_lelang (id_lelang, id_user, penawaran_harga) VALUES (NEW.id_lelang, NEW.id_user, NEW.harga_akhir)
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `status_lelang` AFTER UPDATE ON `tb_lelang` FOR EACH ROW UPDATE tb_barang 
